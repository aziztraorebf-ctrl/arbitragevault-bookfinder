...
async def debug_daily_review(
    days_back: int = Query(default=7),
    db: AsyncSession = Depends(get_db_session),
    current_user: CurrentUser = Depends(get_current_user),
):
    """Temporary debug endpoint to diagnose picks query failure."""
    import traceback
    diagnostics = {}
    cutoff_naive = datetime.utcnow() - timedelta(days=days_back)
    cutoff_aware = datetime.now(timezone.utc) - timedelta(days=days_back)
    diagnostics["cutoff_naive"] = str(cutoff_naive)
    diagnostics["cutoff_aware"] = str(cutoff_aware)

    # Test 1: Raw SQL count
    try:
        r = await db.execute(text(
            "SELECT count(*) FROM autosourcing_picks p "
            "JOIN autosourcing_jobs j ON p.job_id = j.id "
            "WHERE j.created_at >= :cutoff AND p.is_ignored = false"
        ), {"cutoff": cutoff_naive})
        diagnostics["raw_sql_count_naive"] = r.scalar()
    except Exception as e:
        diagnostics["raw_sql_naive_error"] = f"{type(e).__name__}: {e}"

    # Test 2: Raw SQL with aware
    try:
        r = await db.execute(text(
            "SELECT count(*) FROM autosourcing_picks p "
            "JOIN autosourcing_jobs j ON p.job_id = j.id "
        ...
        diagnostics["raw_sql_count_aware"] = r.scalar()
    except Exception as e:
        diagnostics["raw_sql_aware_error"] = f"{type(e).__name__}: {e}"
    ...
    try:
        picks_query = (
            ...
            .where(
                and_(
                    AutoSourcingJob.created_at >= cutoff_naive,
        ...
        diagnostics["orm_count"] = len(rows)
    except Exception as e:
        diagnostics["orm_error"] = f"{type(e).__name__}: {e}"
        diagnostics["orm_traceback"] = traceback.format_exc()

    # Test 4: Simple pick count
    try:
        r = await db.execute(text("SELECT count(*) FROM autosourcing_picks"))
        diagnostics["total_picks"] = r.scalar()
    except Exception as e:
        diagnostics["total_picks_error"] = f"{type(e).__name__}: {e}"
    ...
    try:
        r = await db.execute(text(
            ...
            "WHERE table_name = 'autosourcing_jobs' AND column_name = 'created_at'"
        ))
        row = r.fetchone()
        diagnostics["jobs_created_at_type"] = dict(row._mapping) if row else "NOT FOUND"
    except Exception as e:
        diagnostics["column_type_error"] = f"{type(e).__name__}: {e}"
